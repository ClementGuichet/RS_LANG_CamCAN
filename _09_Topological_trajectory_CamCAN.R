...
trajectory <- function(composition, list_RSN, threshold) {
  ...
  if (composition == "modular") {
    modular <<- tmp_cluster_final %>%
      group_by(Consensus_vector_0.15, Region, Age_group, Subj_ID, MODULAR) %>%
      summarise(n = n()) %>%
      mutate(freq = n / sum(n)) %>%
      dplyr::select(-n) %>%
      spread(MODULAR, freq) %>%
      ...
      arrange(Region) %>%
      pivot_longer(
        cols = !c("Consensus_vector_0.15", "Region", "Age_group"),
        names_to = "MODULAR", values_to = "freq"
      ) %>% 
      mutate(Age_group = ifelse(Age_group == 1, "Young",
                                ifelse(Age_group == 2, "Middle", "Old")))
    ...
    if (list_RSN != "All") {
      Cond_PMF <- modular %>%
        filter(grepl(list_RSN, Consensus_vector_0.15)) %>%
        spread(Age_group, freq) %>%
        arrange(MODULAR) %>%
        group_by(Region) %>%
        group_split()
    ...
    interareal <<- tmp_cluster_final %>%
      group_by(Consensus_vector_0.15, Region, Age_group, Subj_ID, INTERNODAL) %>%
      summarise(n = n()) %>%
      mutate(freq = n / sum(n)) %>%
      dplyr::select(-n) %>%
      spread(INTERNODAL, freq) %>%
...
trajectory("modular", "1", 3)
trajectory("modular", "2", 3)
trajectory("modular", "3", 3)
trajectory("modular", "4", 2)
...
trajectory_inflexion <- function(composition, list_RSN, threshold) {
  ...
  tmp_cluster_final <- tmp_cluster_final %>% 
    mutate(Age_inflexion = ifelse(Age < 55, "Young", "Old"))
...
trajectory_inflexion("modular", "All", 2)
              ...
              ifelse(
                ...
                ifelse(
                  ...
                  ifelse(
                    ...
                    ifelse(
                      Young == "Satellite" & Old == "Provincial", "Periph",
                      ifelse(Young == "Connector" & Old == "Satellite", "Periph",
                        ifelse(Young == "Provincial" & Old == "Satellite", "Inter",
                          "undefined"
                        )
                      )
                    )
...
flexibility_reconfig_abs <- Cond_PMF_final %>% 
  ...
  spread(Age_inflexion, MODULAR) %>% 
  relocate(Old, .after = Young) %>% 
  unite("Topological_mechanism", Young, Old, remove = FALSE) %>% 
  mutate(Topological_mechanism = ifelse(Young == Old, "A", Topological_mechanism)) %>% 
  # mutate(Topological_mechanism = ifelse(
  #   Young == "Provincial" & Old == "Connector", "Inter",
  #   ifelse(
  #     Young == "Satellite" & Old == "Connector", "Intra",
...
ggplot(flexibility_reconfig_abs, aes(`1st_network`, n, fill = forcats::fct_rev(Reconfig))) +
...
flexibility_reconfig_abs_ComStruct <- Cond_PMF_final %>% 
  mutate(Consensus_vector_0.15 = ifelse(Consensus_vector_0.15 == "1", "RS-NET 1\n (DMN, FPN, Language)",
                                        ifelse(Consensus_vector_0.15 == "2", "RS-NET 2\n (CON)",
                                               ifelse(Consensus_vector_0.15 == "3", "RS-NET 3\n (SMN, CON)",
                                                      ifelse(Consensus_vector_0.15 == "4", "RS-NET 4\n (FPN, CON)",
                                                             "RS-NET 5\n (VMM)"
                                                      )
                                               )
                                        )
  ...
  mutate(Topological_mechanism = ifelse(Young == Old, "A", Topological_mechanism)) %>% 
  # mutate(Topological_mechanism = ifelse(
  #   Young == "Provincial" & Old == "Connector", "Gain_Inter",
  #   ifelse(
  #     Young == "Satellite" & Old == "Connector", "Gain_Intra",
  #     ifelse(
  #       Young == "Satellite" & Old == "Peripheral", "Loss_Inter",
  #       ifelse(Young == Old, "A",
  #              ifelse(
  #                Young == "Peripheral" & Old == "Connector", "Super_Gain",
  #                ifelse(
  #                  Young == "Peripheral" & Old == "Satellite", "Gain_Inter",
  #                  ifelse(
  #                    Young == "Peripheral" & Old == "Provincial", "Gain_Intra",
  #                    ifelse(
  #                      Young == "Connector" & Old == "Provincial", "Loss_Inter",
  #                      ifelse(
  #                        Young == "Provincial" & Old == "Peripheral", "Loss_Intra",
  #                        ifelse(
  #                          Young == "Satellite" & Old == "Provincial", "Satel_Prov",
  #                          ifelse(Young == "Connector" & Old == "Satellite", "Loss_Intra",
  #                                 ifelse(Young == "Provincial" & Old == "Satellite", "Prov_Inter",
  #                                        "undefined"
  #                                 )
  #                          )
  #                        )
