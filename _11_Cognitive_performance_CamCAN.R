...
# 
# loadings_y <- ggplot(plot_loadings_y, aes(x = forcats::fct_rev(Cognitive_assessment), y = loading))+
#   geom_col(aes(fill = loading), alpha = .8) +
#   scale_fill_distiller(palette = "RdBu", direction = -1) +
#   coord_flip() +
#   labs(y = "Loadings", x = element_blank()) + 
...
# cca_df_med <- cca_df %>% dplyr::select(Behavioral_Mode, Brain_Mode, Age, Eloc) %>% scale(.) %>% as.data.frame()
# 
...
Data_CCA_ComStruct <- merge(participants, CAMCAN_cognitive_data, by = "Observations") %>%
  ...
  merge(., TFP_ComStruct_imputed_ILR %>% dplyr::select(Subj_ID, Age, Consensus_vector_0.15,
                                            Integration, Segregation, Peripherisation, Polyvalent_interfaces), by = "Subj_ID")
...
Cog_data_ILR_ComStruct <- Data_CCA_ComStruct %>%
  ...
  mutate(Hotel_Task_inverse = Hotel_Task*(-1)) %>%
  dplyr::select(-c(ToT_Ratio, Hotel_Task)) %>%
  pivot_longer(c(Integration, Segregation, Peripherisation, Polyvalent_interfaces),
...
cog_measures_ComStruct <- Cog_data_ILR_ComStruct[,c(5:10, 12:13)] %>% scale(.) %>% as.data.frame()

rs_measures_ComStruct <- Cog_data_ILR_ComStruct[,14:29] %>% scale(.) %>% as.data.frame()
...
plot_ComStruct %>% 
  ...
  labs(x = "Brain Mode", 
       ...
       title = "Canonical correlation between brain/behavioral modes") + 
  annotate("text", fontface = "bold",
           x = -0.125, y = -0.12, label = "Correlation = .35",
           color = "black", size = 4
  ) +
  theme(plot.title.position = "plot") +
  scale_color_viridis(option = "plasma") +
...
mediation_ComStruct <- plot_ComStruct %>% 
  ...
  dplyr::select(Behavioral_Mode, Brain_Mode, Age, Eloc, Balance_eff) %>% scale(.) %>% 
  as.data.frame()
...
write.csv(mediation_ComStruct, 'file_mediation_ComStruct.csv')

labels=list(X="Age",M1="Brain_Mode", M2 = "Balance_eff", Y="Behavioral_Mode")
par(mfrow = c(1,1), mar=c(0,0,0,0), oma=c(0,0,0,0))
pmacroModel(6,labels=labels)
statisticalDiagram(6, labels=labels)
...
mod <- "
# Mediation Effect
Behavioral_Mode ~ b1*Brain_Mode+b2*Balance_eff+c1*Age
Brain_Mode ~ a1*Age
Balance_eff ~ a2*Age+d1*Brain_Mode
ind1:=a1*b1
ind2:=a2*b2
secondInd1:=a1*d1*b2
total1:=c1+a1*b1+a2*b2+a1*d1*b2
...
ncors_seg <- cor(cog_measures_ComStruct, rs_measures_ComStruct %>% dplyr::select(ends_with("Segregation")))
ncors_seg_plot <- ncors_seg %>% as.data.frame() %>%
  mutate_all(., funs(ifelse(. < quantile(ncors_seg, na.rm = TRUE)[4], NA, .))) %>%
  as.matrix()


circlize::chordDiagram(ncors_seg_plot,
                       transparency = .2,
                       link.lwd = 1,    # Line width
                       link.lty = 1,    # Line type
                       link.border = 1)
