...
participant_data <- read_excel("meta_data_645/participant_data.xlsx")[, 1:9] %>%
  dplyr::select(-c("gender_code", "hand")) %>%
  plyr::rename(c("Subject" = "Subj_ID")) %>%
  plyr::rename(c("age" = "Age")) %>%
  replace("Subj_ID", seq_len(645)) %>%
  mutate(Gender_c = ifelse(gender_text == "MALE", -0.5, 0.5))

CAMCAN_cognitive_data <- read_excel("meta_data_645/CognitiveData_CamCAN_Apr2022.xlsx") %>%
  filter(Observations %in% participant_data$Observations) %>%
  dplyr::select(-gender_code) %>%
  plyr::rename(c("Age" = "Age_CogData"))
...
# DIMENSIONS 645 Subjects * 5 thresholds
